...
# Root route — serves index.html for local dev; on Vercel, Vercel's static layer handles this
@app.get("/")
async def read_root():
    html_path = os.path.join(os.path.dirname(__file__), "..", "public", "index.html")
    if os.path.exists(html_path):
        return FileResponse(html_path)
    return {"status": "ok", "message": "LearnForge API is running"}
